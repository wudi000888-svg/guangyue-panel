...
def preflight(args):
    ...
    edition_config({},getattr(args,'edition','lite'),getattr(args,'site_id','default'),getattr(args,'infrastructure_file',None),getattr(args,'role',None),getattr(args,'enrollment_file',None),controller_url=getattr(args,'controller_url',None),connect_token=getattr(args,'connect_token',None))
...
def install(args):
    ...
    try:
        ...
        config=edition_config(config,getattr(args,'edition','lite'),getattr(args,'site_id','default'),getattr(args,'infrastructure_file',None),getattr(args,'role',None),getattr(args,'enrollment_file',None),controller_url=getattr(args,'controller_url',None),connect_token=getattr(args,'connect_token',None))
...
def main():
    ...
    p.add_argument('--role',choices=['standalone','controller','business'],help='defaults: Lite=standalone, Pro=controller; --connect-token opts into managed child mode')
    p.add_argument('--enrollment-file',type=Path)
    p.add_argument('--controller-url',help='HTTPS URL of the Pro master for token-connected sub-sites')
    p.add_argument('--connect-token',help='one-time token generated by the Pro master; replaces --enrollment-file')
    ...
    if args.enrollment_file and args.connect_token:
        raise ValueError('use either --connect-token or --enrollment-file, not both')
    if args.connect_token and args.role not in (None,'business'):
        raise ValueError('--connect-token requires the business role')
    if args.connect_token and not args.controller_url:
        raise ValueError('--connect-token requires --controller-url')
    if args.connect_token and not args.site_id:
        raise ValueError('--connect-token requires --site-id from the master')
