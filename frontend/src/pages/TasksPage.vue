...
<template>
  <TablePageLayout :title="t('任务中心')" :description="t('统一查看任务排队、执行结果与失败原因')">
    ...
    <table><thead><tr><th>{{t('任务类型')}}</th><th>{{t('目标资源')}}</th><th>{{t('状态')}}</th><th>{{t('尝试次数')}}</th><th>{{t('执行结果')}}</th><th>{{t('操作')}}</th></tr></thead>
      <tbody><tr v-for="task in tasks" :key="task.id"><td>{{t(labels[task.kind]||task.kind)}}</td><td><code>{{task.target||'—'}}</code></td><td><span :class="['badge',task.state==='failed'?'danger':task.state==='succeeded'?'success':'neutral']">{{t(labels[task.state]||task.state)}}</span></td><td>{{task.attempts}}</td><td>{{task.error?t(task.error):task.finished?new Date(task.finished*1000).toLocaleString():'—'}}</td><td><button v-if="['queued','running'].includes(task.state)" :disabled="busy" @click="cancel(task.id)"><XCircle :size="14"/>{{t('取消')}}</button></td></tr>
