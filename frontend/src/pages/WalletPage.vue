...
const selected=ref(state.value?.me.id||0),wallet=ref<Wallet|null>(null),items=ref<MoneyTransaction[]>([]),code=ref(''),amount=ref(''),reason=ref(''),kind=ref('credit'),password=ref('');
...
async function adjust(){let value:string;try{value=cents(amount.value)}catch(e){error.value=(e as Error).message;return}const v=await run('/adjust',{user_id:selected.value,amount:value,kind:kind.value,reason:reason.value,password:password.value});password.value='';if(v){amount.value='';reason.value='';notice.value=t('余额调整已完成');await load()}}
...
<template><section class="commerce-page">
 ...
 <form v-if="owner" class="commerce-card" @submit.prevent="adjust"><h2>{{t('人工调整余额')}}</h2><label>{{t('操作类型')}}<select v-model="kind" :aria-label="t('操作类型')"><option value="credit">{{t('人工入账')}}</option><option value="gift">{{t('赠送')}}</option><option value="debit">{{t('人工扣减')}}</option></select></label><label>{{t('金额 / 元')}}<input v-model="amount" inputmode="decimal" required/></label><label>{{t('调整原因')}}<input v-model.trim="reason" maxlength="500" required/></label><label>{{t('管理员当前密码')}}<input v-model="password" type="password" autocomplete="current-password" required/></label><p class="commerce-muted">{{t('调整目标为上方选中的成员，操作会永久记入资金流水')}}</p><button class="primary" :disabled="busy">{{t('确认调整')}}</button></form></div>
