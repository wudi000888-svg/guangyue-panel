...
const {switchSite,state}=usePanelContext();
const isPro=computed(()=>state.value?.system.edition==='pro');
...
const peerForm=reactive({name:'',url:'',token:''}),tokenForm=reactive({name:'',scope:'manage',days:90,forever:false});
...
<template>
  <div class="fleet-page">
    <RouterLink v-if="isPro" to="/fleet">← {{t('返回业务站管理')}}</RouterLink>
    <TablePageLayout :title="t(isPro?'独立面板接入':'配对令牌')" :description="t(isPro?'集中接入和管理不同 VPS 上的独立站点':'生成令牌，安装完成后可由 Pro 主站接管本站')">
      <template #actions><button @click="dialog='token';newToken='' "><KeyRound :size="16"/>{{t('创建接入令牌')}}</button><button v-if="isPro" class="primary" @click="dialog='peer'"><Plus :size="16"/>{{t('接入站点')}}</button></template>
      <template #notice><p class="pool-intro"><Server :size="20"/><span>{{t('本站标识')}} <code>{{siteID}}</code> · {{t('各站点独立保存数据、密钥和代理核心配置')}}</span></p><p v-if="!isPro" class="pool-intro">{{t('Lite 只能生成和撤销配对令牌；粘贴令牌接管站点需要 Pro。本站即使作为子站也保留本地登录、节点和订阅能力。')}}</p><p v-if="error&&!dialog&&!removal" class="error" role="alert">{{t(error)}}</p></template>
      <table v-if="isPro" class="adaptive-table"><thead><tr><th>{{t('站点名称')}}</th><th>{{t('面板地址')}}</th><th>{{t('站点标识')}}</th><th>{{t('连接状态')}}</th><th>{{t('操作')}}</th></tr></thead><tbody>
      ...
      <p v-if="isPro" class="field-help">{{t('Pro 主站可以粘贴其他站点的配对令牌并直接接管；Lite 不提供站点接入和接管入口。')}}</p>
    </TablePageLayout>
    ...
    <Teleport to="body"><div v-if="dialog||removal" class="message-overlay" @click.self="close">
      <form v-if="dialog" class="compose-card" role="dialog" aria-modal="true" :aria-label="t(dialog==='peer'?'接入站点':'创建接入令牌')" @submit.prevent="save">
        ...
        <template v-else-if="!newToken"><label>{{t('名称')}}<input v-model="tokenForm.name" maxlength="64" required/></label><label v-if="isPro">{{t('权限')}}<select v-model="tokenForm.scope"><option value="read">{{t('只读查看')}}</option><option value="manage">{{t('站点管理')}}</option></select></label><p v-else class="field-help">{{t('Lite 令牌固定为站点管理权限，仅用于让 Pro 主站接管本站。')}}</p><label>{{t('有效天数')}}<input v-model.number="tokenForm.days" type="number" min="1" max="365" :disabled="tokenForm.forever" :required="!tokenForm.forever"/></label><label class="inline-check"><input v-model="tokenForm.forever" type="checkbox"/>{{t('永久有效')}}</label><p v-if="tokenForm.forever" class="field-help">{{t('永久令牌不会自动过期，请仅在可信场景使用。')}}</p><p class="field-help">{{t('接入令牌绑定当前管理员。禁用管理员或撤销令牌后，远程访问立即失效。')}}</p></template>
