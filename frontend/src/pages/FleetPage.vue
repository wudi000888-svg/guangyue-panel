...
<template>
  <div class="fleet-page">
    ...
    <TablePageLayout :title="t('本站接入令牌')" :description="t('在其他 Pro 面板接入本站时使用，可随时撤销')">
      <table><thead><tr><th>{{t('名称')}}</th><th>{{t('权限')}}</th><th>{{t('有效期')}}</th><th>{{t('最近使用')}}</th><th>{{t('操作')}}</th></tr></thead><tbody><tr v-for="token in tokens" :key="token.id"><td>{{token.name}}</td><td>{{t(token.scope==='manage'?'站点管理':'只读查看')}}</td><td>{{new Date(token.expires*1000).toLocaleDateString()}}</td><td>{{token.last_used?new Date(token.last_used*1000).toLocaleString():'—'}}</td><td><button @click="removal={kind:'tokens',id:token.id,name:token.name}">{{t('撤销')}}</button></td></tr><tr v-if="!tokens.length"><td colspan="5" class="empty">{{t('尚未创建接入令牌')}}</td></tr></tbody></table>
    </TablePageLayout>
