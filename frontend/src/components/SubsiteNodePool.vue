...
function fill(s:ManagedSite){current.value=s;monthlyBudgetGB.value=(s.monthly_budget||0)/1073741824;cooldownHours.value=(s.cooldown_seconds||86400)/3600;mounts.value=(s.mount?.nodes||[]).map(n=>({...n,group_ids:[...n.group_ids]}));baseline.value=signature();}
...
<fieldset class="pool-fields" :disabled="busy"><details class="target-groups"><summary>{{t('新勾选节点加入')}}：{{groupNames(targetGroups)}}</summary><NodeGroupPicker v-model="targetGroups" :groups="nodeGroups" :exclude-ids="['legacy-private']"/><p class="field-help">{{t('子站节点不能加入默认本地节点组；新建节点组均可选择，只影响新勾选的节点，已挂载节点保留各自分组。')}}</p></details>
