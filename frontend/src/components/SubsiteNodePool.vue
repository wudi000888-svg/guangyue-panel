...
import {defaultMountGroups} from '../lib/mounts';
import {t} from '../i18n';
...
const current=ref<ManagedSite|null>(null),mounts=ref<MountedNode[]>([]),monthlyBudgetGB=ref(0);
...
const signature=()=>JSON.stringify([mounts.value,monthlyBudgetGB.value]);
...
function fill(s:ManagedSite){current.value=s;monthlyBudgetGB.value=(s.monthly_budget||0)/1073741824;mounts.value=(s.mount?.nodes||[]).map(n=>({...n,group_ids:[...n.group_ids]}));baseline.value=signature();}
async function load(){busy.value=true;error.value='';try{const [s,g]=await Promise.all([api<ManagedSite>('/'+props.site.id+'/node-pool'),mainAPI<{groups:NodeGroup[];members:Record<string,GroupMember[]>}>('/node-groups')]);nodeGroups.value=g.groups;groupMembers.value=g.members;fill(s);targetGroups.value=defaultMountGroups(nodeGroups.value);}catch(e){if(!isCancelled(e))error.value=(e as Error).message;}finally{busy.value=false;}}
...
async function save(){if(!current.value?.mount)return;busy.value=true;error.value='';savedNotice.value=false;try{if(!Number.isFinite(monthlyBudgetGB.value)||monthlyBudgetGB.value<0||monthlyBudgetGB.value>1073741824)throw new Error(t('子站月度资源预算无效'));fill(await api<ManagedSite>('/'+props.site.id+'/node-pool','PUT',{revision:current.value.revision,catalog_revision:current.value.mount.catalog.revision,monthly_budget:Math.round(monthlyBudgetGB.value*1073741824),nodes:mounts.value,assignment:'groups'}));savedNotice.value=true;emit('saved');}catch(e){if(!isCancelled(e))error.value=(e as Error).message;}finally{busy.value=false;}}
...
<fieldset class="pool-fields" :disabled="busy"><details class="target-groups"><summary>{{t('新勾选节点加入')}}：{{groupNames(targetGroups)}}</summary><NodeGroupPicker v-model="targetGroups" :groups="nodeGroups" scope="subsite"/><p class="field-help">{{t('子站节点只能加入子站节点组；只影响新勾选的节点，已挂载节点保留各自分组。')}}</p></details>
<label class="monthly-budget">{{t('子站每月资源预算（GiB）')}}<input v-model.number="monthlyBudgetGB" type="number" min="0" step="0.01" required/><small>{{t('0 表示不限；达到上限后主站停止向子站下发代理授权，下一自然月自动恢复。')}}</small></label><p v-if="current.monthly_budget" class="field-help">{{t('本月已使用')}} {{bytes(current.monthly_usage||0)}} / {{bytes(current.monthly_budget)}}</p>
