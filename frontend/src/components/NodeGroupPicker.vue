...
const props=defineProps<{groups:NodeGroup[];members?:Record<string,GroupMember[]>;scope?:'private'|'public'|'subsite';hidePublic?:boolean;excludeIds?:string[]}>();
const panel=inject(panelKey,null),rows=computed(()=>props.members??panel?.groupMembers.value??{});
const hidePublic=computed(()=>!!props.hidePublic||panel?.publicFeaturesEnabled.value===false);
const visibleGroups=computed(()=>props.groups.filter(g=>(!props.scope||g.scope===props.scope)&&!props.excludeIds?.includes(g.id)&&(!hidePublic.value||g.scope!=='public')));
</script>
<template>
<div class="group-picker"><fieldset><legend>{{t('节点组')}}</legend>
 ...
 <p v-if="!visibleGroups.length" class="field-help">{{t('暂无可见节点权限组，请开启公共功能或先创建本地节点组。')}}</p>
 </fieldset></div>
</template>
<style scoped>
