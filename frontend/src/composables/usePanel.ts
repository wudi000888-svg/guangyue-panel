...
const userSearch = ref(""),
  ...
  modal = ref(""),
  editingID = ref(0);
const plans=ref<Plan[]>([]),nodeGroups=ref<NodeGroup[]>([]);
async function loadEntitlements(){try{const [p,g]=await Promise.all([api<{plans:Plan[]}>("/plans"),api<{groups:NodeGroup[]}>("/node-groups")]);plans.value=p.plans;nodeGroups.value=g.groups;}catch(e){if(!isCancelled(e))error.value=(e as Error).message;}}
...
const navGroups = computed(() => {
  ...
  const allowed = new Set(owner.value
    ? ["overview", "clients", "monitor", "users", "plans", "ips", "nodes", "subscription", "settings", "wallet", "shop", "orders", "tickets", "redeem-codes", "messages", "pairing"]
