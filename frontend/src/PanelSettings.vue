...
const props=defineProps<{simpleMode?:boolean;remoteSite?:boolean;publicFeaturesEnabled?:boolean}>();
 ...
 <section class="settings-page">
  ...
  <form v-if="ready" @submit.prevent="save">
   <div class="settings-grid"><div class="settings-sections">
    ...
    <section v-if="!simpleMode" class="settings-card"><header><Settings2 :size="19"/><div><h2>{{t('高级功能显示')}}</h2><p>{{t('按需显示公共代理相关入口，减少日常操作干扰')}}</p></div></header><label class="inline-check"><input :checked="publicFeaturesEnabled" type="checkbox" @change="emit('updatePublicFeaturesEnabled', ($event.target as HTMLInputElement).checked)"/>{{t('显示公共 IP 池、公共节点和公共订阅')}}</label><small>{{t('关闭后公共功能只会从导航和路由中隐藏，已有数据不会删除。')}}</small></section>
   </div><aside class="settings-guide"><Info :size="22"/><h3>{{t('功能分区')}}</h3><dl><div><dt>{{t('业务资源')}}</dt><dd>{{t('本地节点、本地 IP 池及本地订阅，用于稳定业务接入。')}}</dd></div><div v-if="!simpleMode"><dt>{{t('公共代理')}}</dt><dd>{{t('自动采集的 IP 池、公共节点及独立公共订阅。')}}</dd></div><div><dt>{{t('系统管理')}}</dt><dd>{{t('成员权限、面板设置与运行状态。')}}</dd></div></dl><p v-if="!simpleMode">{{t('公共代理采集开关与筛选阈值请在公共 IP 池调整。')}}</p></aside></div>
