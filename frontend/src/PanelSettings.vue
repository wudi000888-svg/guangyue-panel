 ...
 <section class="settings-page">
  ...
  <form v-if="ready" @submit.prevent="save">
   <div class="settings-grid"><div class="settings-sections">
    ...
    <section class="settings-card"><header><LifeBuoy :size="19"/><div><h2>{{t('注册与安全')}}</h2><p>{{t('控制新成员是否可以自行创建账号')}}</p></div></header><label class="inline-check"><input v-model="form.registration_enabled" type="checkbox"/>{{t('开放注册')}}</label><label class="inline-check" :class="{muted:!form.registration_enabled}"><input v-model="form.registration_captcha" type="checkbox" :disabled="!form.registration_enabled"/>{{t('启用滑动验证')}}</label><small>{{t('注册账号默认获得演示套餐，管理员仍可在套餐上架中调整销售规则。')}}</small></section>
    ...
    <section class="settings-card"><header><Settings2 :size="19"/><div><h2>{{t('公共功能显示')}}</h2><p>{{t('按需显示公共代理相关入口，减少日常操作干扰')}}</p></div></header><label class="inline-check"><input :checked="publicFeaturesEnabled" type="checkbox" @change="emit('updatePublicFeaturesEnabled', ($event.target as HTMLInputElement).checked)"/>{{t('显示公共 IP 池、公共节点和公共订阅')}}</label><small>{{t('关闭后公共功能只会从导航和路由中隐藏，已有数据不会删除。')}}</small></section>
   </div><aside class="settings-guide"><Info :size="22"/><h3>{{t('功能分区')}}</h3><dl><div><dt>{{t('业务资源')}}</dt><dd>{{t('本地节点、本地 IP 池及本地订阅，用于稳定业务接入。')}}</dd></div><div><dt>{{t('公共代理')}}</dt><dd>{{t('自动采集的 IP 池、公共节点及独立公共订阅。')}}</dd></div><div><dt>{{t('系统管理')}}</dt><dd>{{t('成员权限、面板设置与运行状态。')}}</dd></div></dl><p>{{t('公共代理采集开关与筛选阈值请在公共 IP 池调整。')}}</p></aside></div>
