...
A network resource console for cross-border commerce teams. Lite defaults to an independent SQLite site; Pro defaults to the master with PostgreSQL, Redis, durable background tasks, unified subscriptions and full control of sub-site permissions. A pairing token can be generated after setup. Lite can issue tokens but only Pro can use one to take over a site. Both editions share Go, Vue 3 and the proxy cores. See [edition selection and Pro setup](docs/EDITIONS.md).
...
Choose `guangyue-panel-lite-…tar.gz` for an independent site or optional managed child, or `guangyue-panel-pro-…tar.gz` for a master. A fresh standalone install needs no registration file. After setup, generate a pairing token in the panel; Lite can only issue/revoke tokens, while Pro can paste a token to take over the site. Use `--enrollment-file /root/enrollment.json` only when enrolling a Lite or Pro child under a master. Child sites use SQLite and retain local login, nodes and subscriptions; upgrades preserve existing roles and data. The controller distributes scoped member credentials and exit bindings, aggregates subscriptions, and reserves per-site quotas. Business sites pull over HTTPS and expire authorization after a 15-minute lease. Quality reports are shared by VLESS/HY2 using the same local exit; speed results remain independent. See the [complete business-site guide](docs/BUSINESS-SITES.md) for installation, quotas, recovery and offline limits.
