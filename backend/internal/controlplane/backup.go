...
func (a *App) backupSnapshot(dir string) error {
	...
	defer cancel()
	if err := a.store.db.Snapshot(ctx, filepath.Join(dir, "panel.db")); err != nil {
		return err
	}
...
func restoreBackup(cfg Config, path string) error {
	...
	if original.RealityPublic != cfg.RealityPublic || original.PublicURL != cfg.PublicURL || original.edition() == "pro" && original.siteID() != cfg.siteID() {
...
func restorePostgres(cfg Config, staged string) error {
	return restorePostgresWithRename(cfg, staged, os.Rename)
}
func restorePostgresWithRename(cfg Config, staged string, rename func(string, string) error) error {
	...
	defer dst.db.Close()
	lease, err := dst.db.AcquireController(ctx)
	if err != nil {
		return err
	}
	defer lease.Close()
	previous, err := os.MkdirTemp(cfg.StateDir, "before-restore-pro-")
	if err != nil {
		return err
	}
	dbDir := filepath.Join(previous, "database")
	...
	if err = dst.db.Snapshot(ctx, filepath.Join(dbDir, "panel.db")); err != nil {
		return err
	}
	src, err := persistence.Open(ctx, staged, persistence.Options{})
	if err != nil {
		return err
	}
	defer src.Close()
	if err = persistence.Copy(ctx, src, dst.db, false); err != nil {
		return err
	}
	if err = installRestore(cfg.StateDir, staged, files, rename); err != nil {
		old, openErr := persistence.Open(ctx, dbDir, persistence.Options{})
		if openErr != nil {
			return fmt.Errorf("restore files failed; database recovery required at %s: %w", previous, errors.Join(err, openErr))
		}
		defer old.Close()
		rollback := persistence.Copy(ctx, old, dst.db, false)
		return fmt.Errorf("Pro restore failed; backup at %s: %w", previous, errors.Join(err, rollback))
	}
	return nil
}
