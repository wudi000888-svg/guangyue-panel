...
func (a *App) sendMessage(w http.ResponseWriter, r *http.Request, actor Record) {
	...
	var id int64
	err = tx.QueryRow("INSERT INTO messages(sender_id,sender_name,title,body,category,created) VALUES(?,?,?,?,?,?) RETURNING id", actor.ID, actor.Username, input.Title, input.Body, input.Category, time.Now().Unix()).Scan(&id)
	if err != nil {
		failure(w, 500, "发送站内信失败")
		return
	}
