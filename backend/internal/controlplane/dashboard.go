...
func (a *App) dashboard(w http.ResponseWriter, r *http.Request, actor Record) {
	...
	cacheAllowed := a.store.logsEnabledLocked()
	cacheRevision := a.store.runtime.Revision
	a.store.runtimeMu.RUnlock()
	cacheKey := "dashboard:" + cacheRevision + ":" + strconv.FormatInt(actor.ID, 10) + ":" + actor.Role + ":" + period + ":" + strconv.FormatInt(offset, 10)
	if cacheAllowed {
		if data, ok := a.cache.Get(r.Context(), cacheKey); ok {
			// Recheck after network I/O so mode changes cannot revive cached history.
			a.store.runtimeMu.RLock()
			valid := a.store.logsEnabledLocked() && a.store.runtime.Revision == cacheRevision
			a.store.runtimeMu.RUnlock()
			if valid {
	...
	if cacheAllowed {
		...
		if valid {
			if b, err := json.Marshal(result); err == nil {
				a.cache.Set(r.Context(), cacheKey, b, 10*time.Second)
			}
		}
	}
