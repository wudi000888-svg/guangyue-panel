...
func (a *App) qualityTest(w http.ResponseWriter, r *http.Request) {
	...
	result, status, err := a.measureResourceQuality(r.Context(), parts[2] == "ips", parts[3])
	if err != nil {
...
func (a *App) measureResourceQuality(ctx context.Context, pool bool, id string) (IPQuality, int, error) {
	if !a.heavyMu.TryLock() {
		return IPQuality{}, 409, errors.New("后台任务繁忙，请稍后重试")
	}
	defer a.heavyMu.Unlock()
	if !a.qualityMu.TryLock() {
		return IPQuality{}, 409, errors.New("已有质量检测正在进行，请稍后重试")
	}
	...
	if n.ID == "" {
		return IPQuality{}, 404, errors.New("资源不存在")
	}
	if !n.Enabled {
