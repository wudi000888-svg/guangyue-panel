...
| `role` | 新安装 Lite: `standalone` / Pro: `controller`；提供 `--connect-token` 时为 `business` | 新子站不需要注册文件；升级保留原角色，子站保留本地 owner 与独立运行能力 |
| `controller_url` | 业务站主控 HTTPS 根地址 | 公网、可信证书、禁止重定向 |
| `connect_token` | 旧拉取式子站首次连接使用的一次性令牌 | 首次成功同步后失效；只存于受保护配置并交换为长期业务凭据 |
...
旧普通订阅与公共订阅继续使用独立路由和令牌；0.25.0 的统一订阅通过明确来源参数合并授权节点，旧公共路由不会扩大范围。连接二维码与分享 URI 都是秘密；日志、备份和截图必须同等保护。

## 令牌管理（0.24.0）

新连接在面板生成和导入，不需修改配置文件。`gys_` 令牌带站点地址与持久化实例身份；令牌哈希、加密连接和暂停状态保存在数据库。`site-local.json` 只用于将原 `business` 子站转为独立本地数据管理，须随状态目录备份；数据库类型和原始配置保持不变。


## 系统设置与注册

从 0.30.13 起，系统设置集中为三个页签：

- **站点与注册**：面板名称、组织、默认语言、联系邮箱、登录公告、开放注册、拼图验证和公共功能显示。
- **交易与接入**：兑换码、工单及支付/邮件接入预留配置。套餐是否可购买统一由“套餐上架”控制。
- **运行与网络**：日志模式、BBR 和 HY2 网络优化。管理远程站点时仅显示该站支持的设置。

切换页签保留未保存内容，各区使用对应的保存按钮。

注册拼图采用每次随机生成的庭院图片，支持桌面鼠标、手机触控、方向键和回车验证。验证码答案只保存在服务端，验证成功后签发绑定当前客户端的一次性注册凭证；验证码和凭证均在 2 分钟后过期。错误尝试消耗当前题目，刷新后重试；接口限制请求频率和未完成验证数量。该验证由面板本地提供，不依赖第三方账户或服务。
