...
广月面板使用 Go 控制面、Vue 3 前端和独立代理核心。Lite 默认作为 SQLite 独立站；Pro 默认作为 PostgreSQL + Redis 主站。使用注册文件的 Lite/Pro 站点作为托管子站运行，同时保留本地 owner、数据和核心配置边界。[版本指南](EDITIONS.md)包含容量、安装、故障行为和群站语义。
