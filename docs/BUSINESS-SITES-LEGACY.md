...
本页保留旧版部署和计量协议说明。0.24.0 新接入请使用[子站令牌管理](BUSINESS-SITES.md)。旧站点仍在同一列表，通过“权限调度”维护；移除无需先清空成员，未结算记录在后台保留，等待撤销确认。以下历史安装参数继续兼容，新的页面不再创建这类站点。
...
- 所有业务站同步主控的常规/无日志运行模式；界面使用统一完整模式。
...
重装、恢复旧业务站备份可能导致计数回退；必须核对主控水位后恢复，不能清零继续续租。已退役用户的必要计量记录会保留，不受无日志模式清理。移除业务站会立即隐藏记录并停止下发授权；必要的计量水位与配额预留保留到子站确认撤销，不卸载远端软件。
