...
- 本地完整 Go race 测试与 vet 通过；新用例覆盖令牌导入、旧令牌兼容、重复连接、默认站点 ID 隔离、自身接入拒绝、加密存储、远程用户操作、停用与恢复、撤销、离线移除和旧计量记录保留。
- 转换回归验证本地账号、凭据、节点权限、SQLite 类型和备份配置保留；旧角色转换后的核心不再等待主站租约。
- 前端 34 项测试、英文文案、Vue/TypeScript 检查和生产构建通过。
- 使用隔离的两个本地站点实际操作：生成并复制永久令牌、Pro 导入、子站用户页切换、暂停/恢复、返回本站和移除后列表刷新。使用合成账号，无真实流量，未以此声称生产代理吞吐验证。
- 本机 macOS 的部署测试有两项 OpenSSL `-checkhost` 平台差异；发布由 Linux CI 的完整部署测试与六组真实安装/升级/回滚矩阵把关。新子站回归同时加入 PostgreSQL CI 行为测试。
...
| 本地 UI | 使用合成账号/流量检查中文/English、统一导航与工作台；没有使用生产截图 |
...
## 0.18.0 密码与移动界面验证（2026-09-10）

- API 回归覆盖 1 位数字、短数字、单个汉字、空格、72 字节边界、空密码与超长输入；现有 bcrypt 密码继续登录，自助改密和管理员重设撤销旧会话，编辑留空保持密码。
- Chromium 在 360 / 390 / 768 / 1440 像素下遍历 17 个页面，使用合成成员、出口、节点、业务站和任务数据检查列表；无整页横向溢出或 JavaScript 页面错误。
- 实际本地 API + 浏览器表单验证创建短密码成员、登录、改密、重新登录和普通成员导航权限。
- 人工查看桌面和手机截图；检查中英文、浅深色、统一分页、导航键盘焦点、滚动锁、从桌面收起切到手机抽屉、抽屉进入更新弹窗后恢复滚动。

浏览器模拟不等同于所有实体手机；输入焦点、软键盘和安全区仍受终端浏览器影响。Linux 安装与升级集成测试继续由 CI 覆盖 Lite、Pro、业务站及两个版本的更新执行器。
