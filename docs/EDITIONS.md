...
两版共享业务规则、Nginx 443 分流和代理核心。Lite 默认是低资源子站；Pro 默认是主站，统一控制多个 VPS 子站，提供连接池、Redis 短缓存与持久化任务。升级 Pro 不改变节点协议、订阅令牌或固定出口策略。

0.23.0 默认群站结构为 **Pro 主站＋Lite 子站**，兼容 Pro 业务站和显式 Lite 独立站，完整安装和授权步骤见[业务站指南](BUSINESS-SITES.md)。主控使用 PostgreSQL/Redis；业务站使用本地 SQLite，不需要为每个 VPS 安装数据库服务。Release assets 分别提供 Lite、Pro 压缩包。
...
| 默认角色 | `business`，受主站统一管理 | `controller`，主站管理入口 |
| 多 VPS | 通过注册文件加入 Pro 主站 | 统一管理最多 64 个 Lite / Pro 子站 |
...
本节只适用于独立 Lite；托管 Lite 子站不得通过普通升级切换 edition 或提升为主站。旧 Lite 原地升级会保留独立角色，新装独立 Lite 需显式指定 `--role standalone`。
