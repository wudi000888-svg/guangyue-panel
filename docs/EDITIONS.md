...
| 多 VPS | Lite 只能生成令牌；使用注册文件成为子站 | 粘贴配对令牌接管，统一管理最多 64 个 Lite / Pro 子站 |
...
独立 Lite/Pro 安装不需要注册文件。登录后可在“配对令牌”页面生成令牌；Lite 只能生成和撤销令牌，不能接管其他站点。Pro 主站在“独立面板接入”粘贴目标站点地址和令牌即可接管。使用 `--enrollment-file` 安装的 Lite/Pro 才是托管子站；子站保留本地 owner、节点、订阅和故障时独立运行能力。托管子站不得通过普通升级切换 edition 或提升为主站，升级会保留原角色。
