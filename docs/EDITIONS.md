...
两版共享业务规则、Nginx 443 分流和代理核心。Lite 默认是低资源独立站；Pro 默认是主站，统一控制多个 VPS 子站，提供连接池、Redis 短缓存与持久化任务。新装站点无需注册文件，Pro 主站生成一次性连接令牌。Pro 的独立站点和托管子站使用同一套站点模型；历史联邦接入站点会在首次打开群站管理时自动转换为子站，保留原节点、订阅、登录和数据。

0.23.1 支持 **Pro 主站＋Lite/Pro 子站**；独立 Lite/Pro 站点是可转换的初始状态，不再作为 Pro 的第二套管理系统。Lite 只能生成配对令牌；Pro 新建子站使用连接令牌，历史独立接入会自动转换。托管子站仍可本地独立运行。完整安装和授权步骤见[业务站指南](BUSINESS-SITES.md)。主控使用 PostgreSQL/Redis；普通业务站使用本地 SQLite，不需要为每个 VPS 安装数据库服务。Release assets 分别提供 Lite、Pro 压缩包。
...
| 多 VPS | Lite 可用连接令牌加入 Pro 主站；仍保留本地 owner | 生成一次性连接令牌，统一管理最多 64 个 Lite / Pro 子站 |
...
所有新安装都不需要注册文件。新建站点在 Pro 主站“子站”页生成一次性连接令牌；目标 Lite/Pro 安装完成后使用 `--controller-url`、`--site-id` 和 `--connect-token` 绑定。此前通过“独立面板接入”连接的站点会在 Pro 主站首次打开“子站”页时自动转换：主站使用原管理令牌向目标站写入托管连接标记，目标站自动重启并主动完成业务连接，原本地 owner、节点、订阅、数据库和代理核心配置保留。主站统一下发成员、额度、节点和出口策略，子站保留本地独立运行能力。旧 `--enrollment-file` 仅作为迁移兼容入口。
...
2. 目标站点 → **群站管理 → 独立面板接入 → 创建接入令牌**，选择“只读查看”或“站点管理”，有效期 1～365 天，或选择“永久有效”。永久令牌不会自动过期，请仅在可信场景使用。令牌绑定创建它的管理员。
...
本节只描述 0.23.3 及更早版本留下的兼容记录。升级后的 Pro 会在统一子站页自动尝试转换；目标站离线或版本过旧时，记录会暂时保留并在下次访问时重试。转换成功后，旧联邦令牌不再作为管理入口，站点由业务令牌、租约和主站权限统一管理。转换不会删除远端数据，也不会把本地 owner 或历史流量合并到主站用户目录。
