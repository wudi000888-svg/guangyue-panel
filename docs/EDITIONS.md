...
两版共享业务规则、Nginx 443 分流和代理核心。Lite 默认是低资源独立站；Pro 默认是主站，统一控制多个 VPS 子站，提供连接池、Redis 短缓存与持久化任务。新装独立站无需注册文件，安装完成后可生成配对令牌。升级 Pro 不改变节点协议、订阅令牌或固定出口策略。

0.23.1 支持 **Pro 主站＋Lite/Pro 子站**，同时保留独立 Lite/Pro 站点。Lite 只有生成配对令牌的能力，Pro 可粘贴令牌直接接管；托管子站仍可本地独立运行。完整安装和授权步骤见[业务站指南](BUSINESS-SITES.md)。主控使用 PostgreSQL/Redis；业务站使用本地 SQLite，不需要为每个 VPS 安装数据库服务。Release assets 分别提供 Lite、Pro 压缩包。
...
所有新安装都不需要注册文件。Pro 主站在“子站”页生成一次性连接令牌；目标 Lite/Pro 安装完成后使用 `--controller-url`、`--site-id` 和 `--connect-token` 绑定。主站统一下发成员、额度、节点和出口策略，子站保留本地 owner、节点、订阅和断网时的独立运行能力。旧 `--enrollment-file` 仅作为迁移兼容入口，托管子站不得通过普通升级切换 edition 或提升为主站，升级会保留原角色。
