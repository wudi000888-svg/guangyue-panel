...
| 多 VPS | 每台单独登录 | HTTPS 接入最多 64 个独立站点 |
...
2. 目标站点 → **群站管理 → 创建接入令牌**，选择“只读查看”或“站点管理”，有效期 1～365 天。令牌绑定创建它的管理员。
3. 主控面板 → **群站管理 → 接入站点**，填目标 HTTPS 根地址和令牌。接入时校验 TLS、地址和站点身份；不得填写内网地址。
...
群站功能是独立站点联邦管理：没有共享用户目录、自动全局配额、负载均衡或跨地域自动故障转移。PostgreSQL advisory lock 防止相同 schema 被两个控制器同时接管；失去控制连接时进程停止，新请求与核心应用会校验控制状态。部署时仍需明确每个站点的唯一负责人，禁止同站点多主部署。SQL schema 是应用隔离边界；不可信租户应使用不同数据库角色、数据库或实例。
