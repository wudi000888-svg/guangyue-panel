...
适用于广月面板 0.18.0。首次部署按本文从上到下执行。已有部署使用[升级流程](OPERATIONS.md)，不要重新运行安装器覆盖数据。
...
```bash
# 在工作电脑或服务器执行。
release_url=https://github.com/wudi000888-svg/guangyue-panel/releases/download/v0.18.0
curl -fL --retry 3 -O "$release_url/guangyue-panel-lite-0.18.0-linux-amd64.tar.gz"
curl -fL --retry 3 -O "$release_url/SHA256SUMS"
# Linux：
sha256sum --ignore-missing -c SHA256SUMS
# macOS 对已下载文件可用 shasum -a 256，并与 SHA256SUMS 对照。
tar -xzf guangyue-panel-lite-0.18.0-linux-amd64.tar.gz
cd guangyue-panel-lite-0.18.0-linux-amd64
```
