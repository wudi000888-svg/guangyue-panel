...
适用于广月面板 0.14.0。首次部署按本文从上到下执行。已有部署使用[升级流程](OPERATIONS.md)，不要重新运行安装器覆盖数据。
...
gh release download v0.14.0 --repo wudi000888-svg/guangyue-panel \
  --pattern 'guangyue-panel-0.14.0-linux-amd64.tar.gz' --pattern 'SHA256SUMS'
# Linux：
sha256sum --ignore-missing -c SHA256SUMS
# macOS 对已下载文件可用 shasum -a 256，并与 SHA256SUMS 对照。
tar -xzf guangyue-panel-0.14.0-linux-amd64.tar.gz
cd guangyue-panel-0.14.0-linux-amd64
```
