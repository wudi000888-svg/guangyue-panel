...
适用于广月面板 0.23.1。首次部署按本文从上到下执行。已有部署使用[升级流程](OPERATIONS.md)，不要重新运行安装器覆盖数据。

Release assets 已拆分 Lite / Pro；包内标记决定默认版本。本文演示独立 Lite（`--role standalone` 可省略）；新装独立站不需要注册文件，登录后可生成配对令牌。使用 `--enrollment-file` 的托管子站安装见[业务站指南](BUSINESS-SITES.md)。安装 Pro 或从 Lite 升级，先阅读[双版本部署指南](EDITIONS.md)。证书、Nginx 与代理核心步骤两版通用。

## 1. 系统、资源与域名

新安装默认启用 BBR 与 HY2 优化，要求宿主机支持并允许修改网络参数。不支持的环境可在安装命令添加 `--no-bbr --no-hy2-optimization`；安装后在 **系统设置 → 网络优化** 调整。参数与恢复行为见[运维指南](OPERATIONS.md#bbr-与-hy2-优化)。
...
```bash
# 在工作电脑或服务器执行。
release_url=https://github.com/wudi000888-svg/guangyue-panel/releases/download/v0.23.1
curl -fL --retry 3 -O "$release_url/guangyue-panel-lite-0.23.1-linux-amd64.tar.gz"
curl -fL --retry 3 -O "$release_url/SHA256SUMS"
# Linux：
sha256sum --ignore-missing -c SHA256SUMS
# macOS 对已下载文件可用 shasum -a 256，并与 SHA256SUMS 对照。
tar -xzf guangyue-panel-lite-0.23.1-linux-amd64.tar.gz
cd guangyue-panel-lite-0.23.1-linux-amd64
```
