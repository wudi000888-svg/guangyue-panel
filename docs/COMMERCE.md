...
适用于 **0.21.0 及以后**，Lite 单机与 Pro 主控站。界面位于侧边栏“账户与服务”，这些核心入口在统一面板中始终可用。普通成员仅查看自己的余额、订单和工单；管理员可以管理所有成员。业务站继续负责代理连接与用量上报。
...
管理员在“系统设置 → 交易与接入 → 在线支付方式”配置支付适配器。当前提供易支付（EPay）跳转下单和通用 JSON Webhook 适配器；密钥使用站点 Vault 加密保存，列表和接口不会回显密钥。一个支付方式可以停用而不影响历史订单。

成员创建订单后可以选择已启用的在线支付方式。订单金额、币种和套餐快照在服务端生成，浏览器回跳不会确认订单；只有支付平台签名验证通过的回调才会把支付尝试标记为已支付并进入套餐开通队列。回调事件按提供商事件号和载荷哈希幂等保存，重复通知不会重复开通。

易支付回调地址为 `https://面板域名/api/payments/webhook/<支付方式 ID>`，支持 GET 和表单 POST，签名采用平台 MD5 规则。通用 Webhook 使用 POST JSON 和 `X-Guangyue-Signature: sha256=<HMAC-SHA256>`，载荷字段为 `event_id`、`merchant_ref`、`external_ref`、`amount`、`currency` 和 `status`。通用 Webhook 用于已有收银台或后续适配器接入，当前不提供跳转下单链接。

在线支付订单不冻结站内余额；开通失败会保留“待人工退款”状态，不会把外部支付误记为站内余额。退款审批仍沿用订单中心流程，当前先退回站内余额，原路退款适配器将在后续版本接入。
...
从 0.21.2 起，顶部直接显示当前登录账号的可用余额，点击可进入“账户余额”。悬停或通过辅助阅读标签可查看冻结余额；手机端同样常驻显示可用余额。管理员在钱包页选择其他成员时，顶部仍显示管理员自己的余额。兑换、人工调整和订单操作后自动刷新，可见页面每 30 秒同步一次，重新进入页面也会同步。加载或读取失败显示“—”，不会把读取失败当成零余额。切换至独立站点的远程管理界面时隐藏本站余额。
...
- 已分配业务站需支持套餐权限与配额周期（协议 2）。旧协议站点需要先升级。从 0.30.13 起，原有预留额度不会阻止套餐上架、购买或续期。更换套餐由系统结算旧授权后重新计算子站额度，无需手动调整；用户套餐额度用尽时撤销该用户节点授权并移出订阅，不删除节点组成员，也不影响其他用户。
- 离线业务站的旧授权须完成撤回/结算后才发新周期，不凭空重新分配预算。新权益提交后节点实际可用性以业务站同步状态为准。
- 无日志模式保留必要资金、订单、工单和授权业务数据；与统一面板界面独立。
